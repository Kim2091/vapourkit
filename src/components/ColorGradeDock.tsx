...
//
// The dock reports the height it wants rather than hard-coding one, because
// four trackballs at full size need about 230px and a short window does not
// have it to give. Below that the balls shrink and the per-ball numbers
// collapse to the master alone — the same rule the design doc works out.
...
export const GRADE_DOCK_COMPACT_HEIGHT = 176;
...
interface ColorGradeDockProps {
  ...
  stepLabel: string;
  compact: boolean;
  /** True when the graph and 2x2 scope grid have their own column beside the viewer. */
  scopesElsewhere: boolean;
  disabled?: boolean;
  dockScope: ScopeKind;
...
export const ColorGradeDock = memo<ColorGradeDockProps>(({
  ...
  const ballSize = compact ? 56 : 84;

  return (
    <div
      className="flex-shrink-0 flex flex-col bg-ink-900 border-t border-ink-700"
      style={{ height: compact ? GRADE_DOCK_COMPACT_HEIGHT : GRADE_DOCK_HEIGHT }}
    >
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-shrink-0 flex flex-col gap-1.5 px-3 py-2 border-r border-ink-850">
          <span className="font-display text-[9px] font-semibold uppercase tracking-[0.13em] text-ink-500 flex items-center gap-1.5">
            Primaries
            <span className="normal-case tracking-normal text-ink-600 font-sans font-normal">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1.5 px-3 py-2 border-r border-ink-850 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-x-5 gap-y-1.5 content-start">
        ...
        {/* One scope, at the end of the dock — Resolve's position for them.
            When the window is wide enough for the graph column, the full 2x2
            grid lives there instead and this end of the dock goes away. */}
        {!scopesElsewhere && (
