...
export const FilterImportModal = memo<FilterImportModalProps>(({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div
        ref={modalRef}
        className="bg-ink-850 rounded-xl border border-ink-800 shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col"
      >
        {/* Header */}
          ...
          <div className="px-4 pt-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-ink-900 border border-ink-700 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-accent-500 transition-colors placeholder-ink-500"
              />
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {importItems.length === 0 ? (
            <div className="text-center text-ink-400 py-12">
              ...
              <p className="text-xs text-ink-500 mt-1">(AI model filters are not imported)</p>
            </div>
            ...
            <>
              ...
              <div className="mb-3 flex items-center justify-between px-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 rounded border-ink-600 bg-ink-900 text-accent-500 
              ...
              <div className="space-y-2">
                {filteredItems.map((item) => {
                  ...
                  <div
                    key={realIndex}
                    className="bg-ink-900 rounded-lg border border-ink-700 p-3 hover:border-ink-600 transition-colors"
                  >
                    {editingIndex === realIndex ? (
                      ...
                      <div className="space-y-2">
                        <div className="flex items-start gap-2">
                          <input
                            ...
                            className="w-4 h-4 mt-1.5 rounded border-ink-600 bg-ink-850 text-accent-500 
                                     focus:ring-2 focus:ring-accent-500 focus:ring-offset-0"
                          />
                          <div className="flex-1 min-w-0">
                            <div className="text-xs text-ink-500 mb-1">Filter Name</div>
                            <div className="flex items-center gap-2 mb-2">
                              <input
                                ...
                                className="flex-1 px-2 py-1.5 bg-ink-850 border border-ink-700 rounded text-sm
                                         focus:outline-none focus:border-accent-500 transition-colors"
                                autoFocus
                              />
                              <button
                                onClick={saveEdit}
                                className="p-1.5 hover:bg-ink-850 rounded transition-colors group"
                                title="Save"
                              >
                                <Check className="w-3.5 h-3.5 text-ok-400 group-hover:text-ok-300" />
                              </button>
                              <button
                                onClick={cancelEdit}
                                className="p-1.5 hover:bg-ink-850 rounded transition-colors group"
                                title="Cancel"
                              >
                                <X className="w-3.5 h-3.5 text-ink-400 group-hover:text-bad-400" />
                              </button>
                            </div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-2">
                                {Array.isArray(item.category) ? (
                                  item.category.map((cat, idx) => (
                                    <span key={idx} className="text-xs px-2 py-0.5 bg-ink-950 text-ink-400 rounded">
                                      {cat}
                                    </span>
                                  ))
                                ) : (
                                  <span className="text-xs px-2 py-0.5 bg-ink-950 text-ink-400 rounded">
                                    {item.category}
                                  </span>
                                )}
                              </div>
                        ...
                        {/* Code Preview */}
                        <div className="ml-6">
                          <div className="text-xs text-ink-500 font-mono bg-ink-850/50 rounded p-2 overflow-x-auto">
                            {item.code.split('\n')[0]}
                            {item.code.split('\n').length > 1 && ' ...'}
                          </div>
                        </div>
                      ...
                      <div className="flex items-start gap-2">
                        <input
                          ...
                          checked={item.selected}
                          onChange={() => toggleSelection(realIndex)}
                          className="w-4 h-4 mt-1 rounded border-ink-600 bg-ink-850 text-accent-500 
                                   focus:ring-2 focus:ring-accent-500 focus:ring-offset-0"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap mb-1">
                                ...
                                {Array.isArray(item.category) ? (
                                  item.category.map((cat, idx) => (
                                    <span key={idx} className="text-xs px-2 py-0.5 bg-ink-950 text-ink-400 rounded flex-shrink-0">
                                      {cat}
                                    </span>
                                  ))
                                ) : item.category ? (
                            ...
                            <button
                              ...
                              className="p-1.5 hover:bg-ink-850 rounded transition-colors group flex-shrink-0"
                              title="Rename filter"
                            >
                              <Edit2 className="w-3.5 h-3.5 text-ink-400 group-hover:text-accent-500" />
                            </button>
                          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm text-ink-400">
              {selectedCount > 0 ? (
                ...
                <span className="text-ink-500">No filters selected</span>
              )}
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 text-xs text-ink-400 mr-2">
                <kbd className="px-2 py-0.5 bg-ink-850 border border-ink-700 rounded text-ink-300">Esc</kbd>
                <span>to close</span>
              </div>
              <button
                onClick={handleCancel}
                className="px-3 py-1.5 bg-ink-850 hover:bg-ink-800 rounded text-sm transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                  selectedCount > 0
                    ...
                    : 'bg-ink-850 text-ink-500 cursor-not-allowed opacity-50'
                }`}
              >
