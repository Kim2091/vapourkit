...
export const UpdateNotificationModal = memo<UpdateNotificationModalProps>(({ 
  ...
  const formatChangelog = (text: string): JSX.Element[] => {
    ...
    return clippedLines.map((line, index) => {
      ...
      if (line.match(/^[\*\-\+]\s/)) {
        return (
          <li key={index} className="text-sm text-ink-300 ml-4 mb-1">
            {line.replace(/^[\*\-\+]\s/, '')}
          </li>
        );
      }
      ...
      if (line.includes('**')) {
        ...
        return (
          <p key={index} className="text-sm text-ink-300 mb-1">
            {parts.map((part, i) => 
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="bg-ink-900 border border-ink-700 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="text-sm text-ink-400">Current Version</p>
                <p className="text-lg font-semibold text-white">v{updateInfo.currentVersion}</p>
              </div>
              <div className="text-accent-500 text-2xl font-bold">→</div>
              <div>
                <p className="text-sm text-ink-400">Latest Version</p>
          ...
          <div className="border-t border-ink-800 pt-4">
            ...
            {changelogExpanded && (
              <div className="bg-ink-900 border border-ink-700 rounded-lg p-4 max-h-64 overflow-y-auto">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-accent-500 to-accent-500 hover:opacity-90 text-white font-semibold rounded-lg px-6 py-3 transition-all duration-300 flex items-center justify-center gap-2 group"
          >
