...
export function useVideoProcessing({
  ...
  useEffect(() => {
    ...
    const basePath = videoInfo.path.replace(/\.[^/.]+$/, '');
    const newPath = `${basePath}-${suffix}.${outputFormat}`;

    if (newPath !== outputPathForComparisonRef.current) {
      setOutputPath(newPath);
      onLog(`Auto-suggested output path updated: ${newPath}`);
    }
  }, [filters, selectedModel, colorimetry, segment, descriptiveNamingEnabled, videoInfo?.path, videoInfo?.outputResolution, outputFormat, isOutputPathAutoGenerated, onLog]);
