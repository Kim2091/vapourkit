...
export const generateTrtexecCommand = (modelType: 'vsr' | 'image', useFp32: boolean, useStaticShape: boolean, inputName: string = 'input', useBf16: boolean = false, temporalFrames: number = 5): string => {
  ...
  if (useStaticShape) {
    // Static shape mode
    return `--shapes=${inputName}:1x${channels}x720x1280 --saveEngine=OUTPUT_PATH --builderOptimizationLevel=3${precisionFlags ? ' ' + precisionFlags : ''}`;
  } else {
    // Dynamic shape mode
    return `--minShapes=${inputName}:1x${channels}x240x240 --optShapes=${inputName}:1x${channels}x720x1280 --maxShapes=${inputName}:1x${channels}x1080x1920 --saveEngine=OUTPUT_PATH --builderOptimizationLevel=3${precisionFlags ? ' ' + precisionFlags : ''}`;
  }
};
